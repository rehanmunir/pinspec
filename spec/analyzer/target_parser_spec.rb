...
RSpec.describe Pinspec::Analyzer::TargetParser do
  ...
  # Both shapes were found by booting real applications: attr_extras accounts for 110
  # of chatwoot's 386 service files, and a module answering its own methods crashed
  # the probe with "undefined method 'new' for module ...".
  describe "a constructor generated by attr_extras" do
    it "reads keyword arguments from the array form, honouring the ! marker" do
      target = described_class.parse(fixture("attr_extras_service.rb"), "perform")

      expect(target.construction_kind).to eq(:new)
      expect(target.initializer_params.map { |p| [p.name, p.kind] })
        .to eq([[:inbox, :keyreq], [:source_ids, :keyreq], [:contact_attributes, :key]])
    end

    it "gives the optional one a default, so it can be omitted" do
      target = described_class.parse(fixture("attr_extras_service.rb"), "perform")
      optional = target.initializer_params.find { |p| p.name == :contact_attributes }

      expect(optional.default_source).to eq("nil")
    end

    it "reads bare symbols as positional arguments" do
      target = described_class.parse(fixture("attr_extras_positional.rb"), "call")

      expect(target.initializer_params.map { |p| [p.name, p.kind] })
        .to eq([[:account, :req], [:params, :req]])
    end
  end

  describe "a module that answers its own methods" do
    it "calls module_function methods on the module, with no construction" do
      target = described_class.parse(fixture("module_function_validator.rb"), "valid?")

      expect(target.construction_kind).to eq(:class_method)
      expect(target.initializer_params).to be_empty
    end

    it "treats `extend self` the same way" do
      target = described_class.parse(fixture("extend_self_registry.rb"), "supported?")

      expect(target.construction_kind).to eq(:class_method)
    end

    # A plain module with neither is not self-calling, and must not be mistaken for one.
    it "does not assume every module answers its own methods" do
      target = described_class.parse(fixture("module_function_validator.rb"), "valid?")

      expect(target).to be_singleton
    end
  end
